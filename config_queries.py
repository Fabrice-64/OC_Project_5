...
query_best_product = """SELECT LEFT(product.name,40), product.brand, product.nutrition_grade, \
        product.code, product.stores FROM category, product \
        WHERE category.name = \"{}\" \
        AND product.name LIKE \"{}\" AND product.nutrition_grade <= \"{}\" \
...
query_retrieve_recorded_product = """SELECT LEFT(product.name, 40), product.brand, \
        product.code, product.nutrition_grade, product.stores, best_product.query_created, \
        (SELECT product.name FROM product WHERE best_product.reference_product = product.code),\
        (SELECT product.brand FROM product WHERE best_product.reference_product = product.code), \
